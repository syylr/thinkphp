           	...
           	<?php 
                if(is_array($info)){
                	foreach ($info as $k=>$val){
                    	echo '<li style="border-bottom:1px solid #EEE; padding:0 12px">' . (is_numeric($k) ? '' : $k.' : ') . htmlentities($val,ENT_COMPAT | ENT_HTML401,'utf-8') .'</li>';
                    }
                }
            ?>
            </ol>
...
<div id="think_page_trace_open" style="text-align: right;overflow:hidden;position:fixed;bottom:0;right:0;color:#000;line-height:30px;cursor:pointer;"><img width="30" style="" title="ShowPageTrace" src="data:image/png;base64,<?php echo App::logo() ?>"><div style="background:#232323;color:#FFF;padding:0 6px;float:right;font-size:14px"><?php echo G('beginTime','viewEndTime').'s ';?></div></div>
