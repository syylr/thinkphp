    ...
    function parseInclude($tmplPublicName){
        if(is_file($tmplPublicName)) {
            ...
            if(substr($tmplPublicName,0,1)=='$'){
                ...
                // 引入其它模块的操作模板
                $tmplTemplateFile   =   dirname(dirname(dirname($templateFile))).'/'.str_replace(array('@',':'),'/',$tmplPublicName);
            }elseif(strpos($tmplPublicName,':')){
                // 引入其它模块的操作模板
                $tmplTemplateFile   =   dirname(dirname($templateFile)).'/'.str_replace(':','/',$tmplPublicName);
            }else{
                // 默认导入当前模块下面的模板
                $tmplTemplateFile = dirname($templateFile).'/'.$tmplPublicName;
            }
