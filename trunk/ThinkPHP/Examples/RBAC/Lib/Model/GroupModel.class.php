...
function setGroupApps($groupId,$appIdList) 
{
    ...
    $rs = $this->db->execute('INSERT INTO '.C("DB_PREFIX").'access (groupId,nodeId,parentNodeId,level) SELECT a.id, b.id,b.pid,b.level FROM '.C("DB_PREFIX").'group a, '.C("DB_PREFIX").'node b WHERE '.$where);
    if($result===false) {
        return false;
    }else {
        return true;
...
{
    $table = C("DB_PREFIX").'access';
    $rs = $this->db->query('select b.id,b.title,b.name from '.$table.' as a ,'.C("DB_PREFIX").'node as b where a.nodeId=b.id and  b.pid='.$moduleId.' and  a.groupId='.$groupId.' ',false);
    return $rs;
}
...
{
    $table = C("DB_PREFIX").'access';
    $rs = $this->db->query('select b.id,b.title,b.name from '.$table.' as a ,'.C("DB_PREFIX").'node as b where a.nodeId=b.id and  b.pid=0 and a.groupId='.$groupId.' ',false);
    return $rs;
...
{
    $table = C("DB_PREFIX").'access';
    $rs = $this->db->query('select b.id,b.title,b.name from '.$table.' as a ,'.C("DB_PREFIX").'node as b where a.nodeId=b.id and  b.pid='.$appId.' and a.groupId='.$groupId.' ',false);
    return $rs;
	//return $this->rsToVo($rs);
}
...
{
    $table = C("DB_PREFIX").'groupUser';
    $rs = $this->db->query('select b.id,b.account from '.$table.' as a ,'.C("DB_PREFIX").'user as b where a.userId=b.id and  a.groupId='.$groupId.' ',false);
    return $rs;
	//return $this->rsToVo($rs);
}
...
function setGroupUsers($groupId,$userIdList) 
{
    ...
    $where = 'a.id ='.$groupId.' AND b.id in('.$id.')';
    $rs = $this->db->execute('INSERT INTO '.C("DB_PREFIX").'groupuser (groupId,userId) SELECT a.id, b.id FROM '.C("DB_PREFIX").'group a, '.C("DB_PREFIX").'user b WHERE '.$where);
    if($result===false) {
        return false;
    }else {
        return true;
