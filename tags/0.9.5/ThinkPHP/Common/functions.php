...
// +----------------------------------------------------------------------+
// $Id: functions.php 12 2007-01-06 03:34:59Z liu21st $
 ...
 * @author     liu21st <liu21st@gmail.com>
 * @version    $Id: functions.php 12 2007-01-06 03:34:59Z liu21st $
 +------------------------------------------------------------------------------
 */
/**
 +----------------------------------------------------------
...
function throw_exception($msg,$type='FcsException',$code=0)
{
    if(isset($_REQUEST['ajax']) || (isset($_REQUEST['_AJAX_SUBMIT_']) && $_REQUEST['_AJAX_SUBMIT_']) ) {
        ...
        exit("<div style='color:#FF3300'><IMG SRC='".APP_PUBLIC_URL."/images/update.gif' align='absmiddle' BORDER='0'> $msg</div>");
    }
...
function encode_file_contents($filename,$strip=true) 
{
    ...
    if('php'==$type && is_file($filename) && is_writeable($filename)) {
        ...
        // 判断文件是否已经被加密
        $pos = strpos($contents,'/*Protected by FCS Cryptation*/');
        if( false === $pos  || $pos>100 ) {
            ...
            $encode   = '<?php'." /*Protected by FCS Cryptation*/\n\$cryptCode='".$encode."';eval(gzinflate(base64_decode(\$cryptCode)));\n /*Reverse engineering is illegal and strictly prohibited - (C) FCS Cryptation 2006*/\n?>";
            // 重新写入加密内容
 ...
 * @return boolen
 +----------------------------------------------------------
 */
function using($class,$baseUrl = LIB_PATH,$appName=APP_NAME,$ext='.class.php',$subdir=false)
{
    return import($class,$baseUrl,$appName,$ext,$subdir);
}
