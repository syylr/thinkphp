...
*/
	...
	Function halt($fStr) {
		if(DEBUG_MODE){//调试模式下输出错误信息
			$output = "<div align='center' id=debug style='vertical-align:middle;width:100%;height:100%;'><div style='width:56%;padding:10;border:1pt dotted gray;background:#FFFFEC;font-size:9pt;line-height:150%;color:orangered;'>"
				.nl2br($fStr);
			if(PHP_VERSION <'5.0.0') $output .= TraceToString();
			$output	.="</div></div>";
			exit($output);
		}else {//否则定向到错误页面
			if(ERROR_PAGE_URL!=''){
	...
	function TraceToString() {
		...
		foreach($debug as $t)
		{
			$trace .= '<div style="padding-left: 20px;">';
			$trace .= '#'.$i.':'.$t['file'].'('.$t['line'].')';
	...
	function ThrowException($msg,$file='')
	{
		if(PHP_VERSION < '5.0.0'){
			...
			//throw new FCSException($msg);
			$my = & new FCSException($msg);
			halt($my->__toString());
		}
	}
...
*/
	...
	function import($class,$baseUrl = LIB_PATH){
		  ...
		  if(array_pop($class_strut) == "*"){
			...
			//导入目录下的指定类库文件
			$classfile = str_replace('.', '/', $baseUrl.$class).'.class.php';
