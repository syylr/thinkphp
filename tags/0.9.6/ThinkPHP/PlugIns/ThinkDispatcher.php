<?php 
/*
Plugin Name: ThinkDispatcher
Plugin URI: http://thinkphp.cn
Description: URL 调度器 需要在配置文件中设置DISPATCH_NAME为ThinkDispatcher才能生效
Author: 流年
Version: 1.0
...
//--------------------------------------------------
// DISPATCH_NAME 设置为 ThinkDispatcher 
//--------------------------------------------------
                ...
                for($i = 0, $cnt = count($paths); $i <$cnt; $i++){
                    if(isset($paths[$i+1])) {
                    	$pathInfo[$paths[$i]] = (string)$paths[++$i];
                     	...
                     	$pathInfo[$pathInfo[VAR_ACTION]] = (string)$paths[$i];
                    }
                }
            }
            else {
...
if(defined('DISPATCH_ON') && DISPATCH_ON) {
    //增加调度器总开关
    if(defined('DISPATCH_NAME') && 'ThinkDispatcher'==DISPATCH_NAME ) {
        // 添加默认的THINKDispatcher调度器
        add_filter('app_dispatch',array('ThinkDispatcher','dispatch'));
    }	
}
