...
// +----------------------------------------------------------------------+
// $Id: ThinkTemplate.class.php 33 2007-02-25 07:06:02Z liu21st $
 ...
 * 支持缓存和页面压缩
 +------------------------------------------------------------------------------
 * @author liu21st <liu21st@gmail.com>
 * @version  $Id: ThinkTemplate.class.php 33 2007-02-25 07:06:02Z liu21st $
 +------------------------------------------------------------------------------
 */
...
{//类定义开始
    ...
    function loadTemplate ($tmplTemplateFile='',$charset=OUTPUT_CHARSET)
    {
        ...
        if(!file_exists($tmplTemplateFile)){
            $tmplTemplateFile =  dirname(TMPL_FILE_NAME).'/'.$tmplTemplateFile.TEMPLATE_SUFFIX;
            if(!file_exists($tmplTemplateFile)){
    ...
    function parseInclude($tmplPublicName){
        if(is_file($tmplPublicName)) {
            ...
            $tmplTemplateFile = dirname($this->templateFile).'/';//TEMPLATE_PATH.'/'.MODULE_NAME.'/';
