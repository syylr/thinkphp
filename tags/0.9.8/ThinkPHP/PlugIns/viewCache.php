...
*/ 
    ...
    {             
         ...
         $cacheInfo  =  $cache->get(MODULE_NAME.'_'.ACTION_NAME.'_CACHE');
         if($cacheType   = $cacheInfo['type']) {
             ...
             if($cacheType=='ALL') { 
                ...
                if(file_exists($cacheFile) && time() <= filemtime($cacheFile)+$cacheTime) {
                 ...
                 //局部视图缓存
                // 获取视图缓存数据
                $view_cache   =  $cache->get(MODULE_NAME.'_'.ACTION_NAME.'_DATA');
                if($view_cache) {
                    // 存在视图缓存数据
            ...
            if($cacheInfo['default'] == 'NOCACHE') {
                ...
                if($find) {
                    ...
                    foreach($matches as $key=>$match) {
                        ...
                        // 记录视图缓存数据
                        $cacheData[] =   $array['cache'];
                    } 
                    // 记录所有的视图缓存数据
                    $cache->set(MODULE_NAME.'_'.ACTION_NAME.'_DATA',$cacheData,-1);
                }  
            }elseif($cacheInfo['default'] ==   'CACHE') {
        ...
        // 生成视图缓存数据
         ...
         if($cacheType   =  $cacheInfo['type']) {
            ...
            if($cacheType=='ALL') {
                ...
                if($cacheInfo['default']=='NOCACHE') {
                    // 生成局部视图缓存文件
                    $cacheData    = $cache->get(MODULE_NAME.'_'.ACTION_NAME.'_DATA');
