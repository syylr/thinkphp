...
class HtmlCache extends Base
{
    ...
    function readHTMLCache()
    {
         if(file_exists(CONFIG_PATH.'htmls.php')) {
            ...
            if(!empty($html)) {
                ...
                HtmlCache::$cacheTime = $time;
                $cacheName  =   $rule.C('HTML_FILE_SUFFIX');
                HtmlCache::$cacheFile = $cacheName;
    ...
    {
        if(HtmlCache::$requireCache) {
            //静态文件写入
            // 如果开启HTML功能 检查并重写HTML文件
            // 没有模版的操作不生成静态文件
            if(MODULE_NAME != 'Public' && !self::checkHTMLCache(HtmlCache::$cacheFile,HtmlCache::$cacheTime)) {
