...
// +--------------------------------------------------------
// |FCS是一个快速、兼容而且简单的面向对象的轻量级PHP开发框架
// |全称是 Fast,Compatible & Simple OOP PHP Framework。
// +--------------------------------------------------------

FCS是一个开源的轻量级PHP框架， FCS框架是为了简化企业级应用开发而诞生的。使用FCS，你可以更方便和快捷的开发和部署应用，并且具有更强大的可维护性、可扩展性以及安全性。当然不仅仅是企业级应用，任何PHP应用开发都可以从FCS的简单、兼容和快速的特性中受益。简洁、快速和实用是FCS发展秉承的宗旨，为此FCS会不断吸收和融入更好的技术以保证其新鲜和活力！

[基本信息]
官方支持Blog：http://fcs.org.cn/blog
官方论坛：http://fcs.org.cn/forum
SVN地址：http://fcs.org.cn/svn/trunk
...
[具备条件]
FCS可以运行在windows/Unix主机上面（PHP的Windows CGI安装模式下面运行不正常）
支持以下数据库（可扩展）
Mysql（Mysqli）
Mssql
Oracle
...
[安装说明]
把FCS、FCSAdmin、Public目录直接上传（或拷贝到）服务器web目录下面
导入FCSAdmin目录下面的fcs.sql数据库脚本到mysql数据库
（如果是mysql4.1以下版本，请修改fcs.sql文件，把DEFAULT CHARSET=utf8去掉）
注意示例程序mysql保存数据采用utf-8编码，请注意mysql相应设置。
修改FCSAdmin/Conf/下面的配置文件设置好数据库访问信息（详见配置说明）
...
配置完成后运行 FCSAdmin目录下面的index.php文件就可以运行示例项目。
后台初始登录帐户名 admin 
密码 admin

[示例项目admin功能说明]
该项目只是一个简单的示例程序，使用了mysql数据库，从目前的应用项目中简化而来，并非真正的FCS后台管理系统。
示例程序演示了如下功能：
登录验证
用户管理
节点管理
