...
class Model extends Base
{
    ...
    var $error = '';
    // 验证错误信息
    var $validateError    =   array();

    // 包含的聚合对象
    var $aggregation = array();
    // 是否为复合对象
    var $composite = false;
    // 是否为视图模型
    var $viewModel = false;
    ...
    // 悲观锁
    var $pessimisticLock = false;

    var $autoSaveRelations      = false;        // 自动关联保存
    var $autoDelRelations        = false;        // 自动关联删除
    var $autoAddRelations       = false;        // 自动关联写入
    var $autoReadRelations      = false;        // 自动关联查询
    var $lazyQuery                =   false;                  // 是否启用惰性查询
     ...
     * @return false|integer
     +----------------------------------------------------------
     */
    function _create(&$data,$autoLink=false,$multi=false) {
        // 前置调用
    ...
    function _before_create(&$data) {return true;}
    function _after_create(&$data) {}
     ...
     * @return boolean
     +----------------------------------------------------------
     */
    function _update(&$data,$where='',$limit='',$order='',$autoLink=false,$lock=false) {
    ...
    // 更新回调方法
    function _before_update(&$data,$where) {return true;}
    function _after_update(&$data,$where) {}
     ...
     * @return boolean
     +----------------------------------------------------------
     */
    function _read($condition='',$fields='*',$all=false,$order='',$limit='',$group='',$having='',$join='',$cache=false,$relation=false,$lazy=false,$lock=false) {
        $table      =   $this->getTableName();
        if(!empty($this->options)) {
            // 已经有定义的查询表达式
    ...
    // Read回调方法
    function _before_read(&$condition) {return true;}
    function _after_read(&$condition,$result) {}
     ...
     * @return boolean
     +----------------------------------------------------------
     */
    function _delete($data,$where='',$limit=0,$order='',$autoLink=false) {
        $table      =   $this->getTableName();
        if(!empty($this->options)) {
            // 已经有定义的查询表达式
            $where      =   isset($this->options['where'])?     $this->options['where']:    $where;
    ...
    // Delete回调方法
    function _before_delete(&$where) {return true;}
    function _after_delete(&$where) {}
     ...
     * @return mixed
     +----------------------------------------------------------
     */
    function _query($sql='',$cache=false,$lazy=false,$lock=false) {
        if(!empty($this->options)) {
    ...
    // Query回调方法
    function _before_query(&$sql) {return true;}
    function _after_query(&$result) {}
     ...
     * @return void
     +----------------------------------------------------------
     */
    function getListBlobFields(&$resultSet,$field='') {
     ...
     * @return void
     +----------------------------------------------------------
     */
    function getBlobFields(&$data,$field='') {
        if(!empty($this->blobFields)) {
            $pk =   $this->getPk();
     ...
     * @return void
     +----------------------------------------------------------
     */
    function delBlobFields(&$data,$field='') {
        if(!empty($this->blobFields)) {
            $pk =   $this->getPk();
            $id =   is_array($data)?$data[$pk]:$data->$pk;
            if(empty($field)) {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function add($data=null,$autoLink=false,$multi=false)
    {
        if(empty($data)) {
            // 没有传递数据，获取当前数据对象的值
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function addAll($dataList='',$autoLink=false)
    {
        if(empty($dataList)) {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function save($data=null,$where='',$autoLink=false,$limit=0,$order='')
    {
        if(empty($data)) {
            if(!empty($this->options['data'])) {
                $data    =   $this->options['data'];
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function checkLockVersion(&$data,$where='') {
        $pk =   $this->getPk();
        $id =   $data[$pk];
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function getRelation(&$result,$name='',$return=false)
    {
        if(!empty($this->_link)) {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function getRelations(&$resultSet,$name='') {
        // 获取记录集的主键列表
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function opRelation($opType,$data='',$name='')
    {
        $result =   false;
        // 把HashMap对象转换成数组
        if(is_instance_of($data,'HashMap')){
     ...
     * @return boolen
     +----------------------------------------------------------
     */
    function deleteById($id,$autoLink=false)
    {
        $pk =   $this->getPk();
     ...
     * @return boolen
     +----------------------------------------------------------
     */
    function deleteByIds($ids,$limit='',$order='',$autoLink=false)
    {
     ...
     */
    // 根据某个字段的值删除记录
    function deleteBy($field,$value,$limit='',$order='',$autoLink=false) {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function delete($data=null,$limit='',$order='',$autoLink=false)
    {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function deleteAll($condition='',$autoLink=false)
    {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function getById($id,$fields='*',$cache=false,$relation=false,$lazy=false)
    {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function getByIds($ids,$fields='*',$order='',$limit='',$cache=false,$relation=false,$lazy=false)
    {
        if(is_array($ids)) {
            $ids    =   implode(',',$ids);
        }
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function getBy($field,$value,$fields='*',$cache=false,$relation=false,$lazy=false)
    {
        $condition[$field]  =  $value;
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function getByAll($field,$value,$fields='*',$cache=false,$relation=false,$lazy=true)
    {
        $condition[$field]  =  $value;
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function find($condition='',$fields='*',$cache=false,$relation=false,$lazy=false)
    {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function xFind($condition='',$fields='*',$cache=false,$lazy=false)
    {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function findAll($condition='',$fields='*',$order='',$limit='',$group='',$having='',$join='',$cache=false,$relation=false,$lazy=false)
    {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function xFindAll($condition='',$fields='*',$order='',$limit='',$group='',$having='',$join='',$cache=false)
    {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function topN($count,$condition='',$fields='*',$order='',$group='',$having='',$join='',$cache=false,$relation=false,$lazy=false) {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function query($sql,$cache=false,$lazy=false)
    {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function getField($field,$condition='')
    {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function getFields($field,$condition='',$sepa=' ')
    {
        if(empty($condition) && isset($this->options['where'])) {
            $condition   =  $this->options['where'];
        }
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function getCols($rs,$field,$sepa=' ') {
        if(!empty($rs)) {
            $field  =   explode(',',$field);
            $cols    =   array();
            $length  = count($field);
     ...
     * @return integer
     +----------------------------------------------------------
     */
    function count($condition='',$field='*')
    {
     ...
     * @return mixed
     +----------------------------------------------------------
     */
    function getN($position=0,$condition='',$order='',$fields='*',$relation=false)
    {
        $table      =   $this->getTableName();
        if(!empty($this->options)) {
            // 已经有定义的查询表达式
     ...
     * @return mixed
     +----------------------------------------------------------
     */
    function first($condition='',$order='',$fields='*',$relation=false) {
     ...
     * @return mixed
     +----------------------------------------------------------
     */
    function last($condition='',$order='',$fields='*',$relation=false) {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function rsToVo($resultSet,$returnList=false,$position=0,$relation='')
    {
        if($resultSet ) {
            if(!$returnList) {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function autoOperation(&$data,$type) {
        // 自动填充
        if(!empty($this->_auto)) {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function autoValidation($data,$type) {
        // 属性验证
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function _validationField($data,$val) {
        // 检查附加规则
        switch($val[4]) {
    ...
    // 表单验证回调方法
    function _before_validation(&$data,$type) {return true;}
    function _after_validation(&$data,$type) {return true;}

    // 表单处理回调方法
    function _before_operation(&$data) {}
    function _after_operation(&$data) {}
     ...
     * @return boolean
     +----------------------------------------------------------
     */
    function addConnect($config,$linkNum=NULL) {
     ...
     * @return boolean
     +----------------------------------------------------------
     */
    function switchConnect($linkNum) {
        if(isset($this->_db[$linkNum])) {
            // 在不同实例直接切换
     ...
     * @return array
     +----------------------------------------------------------
     */
    function sortBy($field, $sortby='asc', $list='' ) {
