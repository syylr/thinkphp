   ...
   /**
    ...
    function showTime($startTime) {
        if(C('SHOW_RUN_TIME')) {
            ...
            if(C('SHOW_DB_TIMES') && class_exists('Db') ) {
                // 显示数据库操作次数
                $db =   Db::getInstance();
                $showTime .= ' | DB :'.$db->Q().' queries '.$db->W().' writes ';
            }
            if(C('SHOW_CACHE_TIMES') && class_exists('Cache')) {
                // 显示数据库操作次数
     ...
     * @return void
     +----------------------------------------------------------
     */
    function layout($layoutFile,$charset='',$contentType='text/html',$varPrefix='',$display=true)
    {
