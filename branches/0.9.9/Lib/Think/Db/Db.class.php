...
class Db extends Base
{
    ...
    // 数据库表达式
    var $comparison      = array('eq'=>'=','neq'=>'!=','gt'=>'>','egt'=>'>=','lt'=>'<','elt'=>'<=','like'=>'LIKE','between'=>'BETWEEN','notnull'=>'IS NOT NULL','null'=>'IS NULL');
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function parseConfig($db_config='') {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function multiConnect($master=false) {
    ...
    function debug() {
        ...
        if ( $this->debug || C('SQL_DEBUG_LOG'))    {
            ...
            Log::write(" RunTime:".$runtime."s SQL = ".$this->queryStr,SQL_LOG_DEBUG);
        }
    }
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function query($sql='',$cache=false,$lazy=false,$lock=false)
    {
        if(!empty($sql)) {
            $this->queryStr = $sql;
        }
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function execute($sql='',$lock=false)
    {
        if(empty($sql)) {
            $sql  = $this->queryStr;
        }
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function autoExec($sql='',$lazy=false,$lock=false,$cache=false)
    {
        if(empty($sql)) {
            $sql  = $this->queryStr;
        }
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function find($where,$tables,$fields='*',$order=null,$limit=null,$group=null,$having=null,$join=null,$cache=false,$lazy=false,$lock=false)
    {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function add($map,$table,$multi=false)
    {
        if($multi) {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function save($sets,$table,$where,$limit=0,$order='',$lock=false)
    {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function setField($field,$value,$table,$condition,$asString=false) {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function setInc($field,$table,$condition,$step=1) {
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    function setDec($field,$table,$condition,$step=1) {
        return $this->setField($field,'('.$field.'-'.$step.')',$table,$condition);
    }
